...
/*!
 * @brief Callback to find the URN associated to an URI
 *
 * @param [in] uri URI to find
 * @param [out] urnP urn of the MO
 * @param [in] data MO internal data as created by #omadm_mo_init_fn
 *
 * @returns a MO error code
 */
typedef int (*omadm_mo_get_urn_fn) (const char * uri, char ** urnP,
                                    void * data);
...
typedef struct
{
    ...
    omadm_mo_get_urn_fn   getURNFunc;
