...
{
    if((NULL == dataP) || (NULL == origP))
    {
        DM_LOGE("set_pcdata_pcdata(): NULL pointer in input params!");
        return;
    }

    dataP->contentType = origP->contentType;

    if ((origP->length > 0) && (origP->content != NULL))
    {
                             ...
                             SmlGenericCmdPtr_t pContent)
{
    ...
    if ((pContent != NULL) && (pContent->cmdID == NULL))
    {
       DM_LOGE("Generic command malformed! cmdID is NULL");
    }
