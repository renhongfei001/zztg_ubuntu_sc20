...
#define PRV_DEFAULT_NONCE "0000000000000000"
...
//TODO: check if needed
    ...
    *serverID = (char *)calloc(field+1,sizeof(char));
    if (!(*serverID))
    {
                       ...
                       buffer_t pkg0)
{
    ...
    DM_LOGD("validate_package_0 got key %s", key);
           ...
           && result != OMADM_SYNCML_ERROR_INVALID_CREDENTIALS)
    {
        ...
        DM_LOGD("validate_package_0 got nonce = %s", internP->account->toClientCred->data.buffer);
        ...
        if (memcmp(pkg0.buffer, H_key, PRV_MD5_DIGEST_LEN))
        {
            if (result == OMADM_SYNCML_ERROR_COMMAND_FAILED)
            {
                ...
                free(H_key);
                H_key = NULL;
                internP->account->toClientCred->data.buffer = (uint8_t *)strdup(PRV_DEFAULT_NONCE);
                internP->account->toClientCred->data.len = strlen(PRV_DEFAULT_NONCE);
                DM_LOGD("OMADM_SYNCML_ERROR_COMMAND_FAILED");
            }
            else
            {
                result = OMADM_SYNCML_ERROR_INVALID_CREDENTIALS;
    ...
    DM_LOGD("validate_package_0 result = <%d>", result);
