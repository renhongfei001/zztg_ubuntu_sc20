...
class TelephonyIcons {
    ...
    static String[] mDataTypeArray, mDataTypeGenerationArray,mDataTypeGenerationArray4g,
                    mDataTypeGenerationArray3g,mDataTypeGenerationArray2g;
    ...
    static void readIconsFromXml(Context context) {
        ...
        try {
            ...
            mDataTypeGenerationArray4g=mRes.getStringArray(
                R.array.telephony_data_type_generation_4g);
            mDataTypeGenerationArray3g=mRes.getStringArray(
                R.array.telephony_data_type_generation_3g);
            mDataTypeGenerationArray2g=mRes.getStringArray(
                R.array.telephony_data_type_generation_2g);
    ...
    static void updateDataTypeForVerizon(int slot, int type, boolean showAtLeast3G,
            boolean show4GforLte, boolean hspaDistinguishable, int inet,MobileSignalController.MobileState currentState,
            boolean showDataIcon) {
        log(TAG, "updateDataTypeForVerizon "
                + String.format("slot=%d, type=%d, inetCondition=%d",
                        slot, type, inet)
                + " showAtLeast3G=" + String.valueOf(showAtLeast3G)
                + " show4GforLte=" + String.valueOf(show4GforLte)
        ...
        switch (type) {
            ...
            case TelephonyManager.NETWORK_TYPE_EDGE:
                if(!showDataIcon){
                    if(!currentState.dataConnected){
                    mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                            mDataTypeGenerationArray2g[5], null, NS);
                    }else{
                    mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                            mDataTypeGenerationArray2g[0], null, NS);
                    }

                    }else{
                  switch(currentState.dataActivity){
                      case TelephonyManager.DATA_ACTIVITY_NONE:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray2g[1], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_IN:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray2g[2], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_OUT:
                         mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray2g[3], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_INOUT:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                   mDataTypeGenerationArray2g[4], null, NS);
                          break;
                  }
                }
                if (!showAtLeast3G) {
            ...
            case TelephonyManager.NETWORK_TYPE_TD_SCDMA:
               ...
               // mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                ...
                if(!showDataIcon){
                    if(!currentState.dataConnected){
                    mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                             mDataTypeGenerationArray3g[5], null, NS);
                    }else{
                    mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                            mDataTypeGenerationArray3g[0], null, NS);
                        }
                    }else{
                  switch(currentState.dataActivity){
                      case TelephonyManager.DATA_ACTIVITY_NONE:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray3g[1], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_IN:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray3g[2], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_OUT:
                         mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                  mDataTypeGenerationArray3g[3], null, NS);
                          break;
                      case TelephonyManager.DATA_ACTIVITY_INOUT:
                          mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                   mDataTypeGenerationArray3g[4], null, NS);
                          break;
                  }
                }
                ...
                if (!show4GforLte || MobileSignalController.isCarrierOneSupported()) {
                    ...
                    if ( type == TelephonyManager.NETWORK_TYPE_LTE_CA) {
                        ...
                        if(!showDataIcon){
                            if(!currentState.dataConnected){
                                mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                        mDataTypeGenerationArray4g[5], null, NS);
                            }else{
                            mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                    mDataTypeGenerationArray4g[0], null, NS);
                            }
                        }else{
                        switch(currentState.dataActivity){
                            case TelephonyManager.DATA_ACTIVITY_NONE:
                                mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                        mDataTypeGenerationArray4g[1], null, NS);
                                break;
                            case TelephonyManager.DATA_ACTIVITY_IN:
                                mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                        mDataTypeGenerationArray4g[2], null, NS);
                                break;
                            case TelephonyManager.DATA_ACTIVITY_OUT:
                               mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                        mDataTypeGenerationArray4g[3], null, NS);
                                break;
                            case TelephonyManager.DATA_ACTIVITY_INOUT:
                                mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                                         mDataTypeGenerationArray4g[4], null, NS);
                                break;
                        }
                            }
                        mSelectedQSDataTypeIcon[slot] = QS_DATA_4G;
        ...
        log(TAG, "updateDataTypeForVerizon " + String.format(
                "mSelectedDataTypeIcon[%d]=%d, mSelectedDataActivityIndex=%d",
                slot, mSelectedDataTypeIcon[slot], mSelectedDataActivityIndex[slot]));
    }
