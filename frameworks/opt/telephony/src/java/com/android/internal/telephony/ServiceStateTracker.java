...
public class ServiceStateTracker extends Handler {
    ...
    protected static final int EVENT_SET_NET_MODE_GLOBAL               = 52;
    protected static final int EVENT_SET_ALERT_NET_GLOBAL              = 53;
    protected static final int EVENT_SET_CLIR                          = 54;
   ...
   private ContentObserver mPreferedNetworkModeObserver = new ContentObserver(new Handler()) {
       @Override
       public void onChange(boolean selfChange) {
           Rlog.i(LOG_TAG, "Prefered Network Mode changed");
    ...
    private final String mVoiceStateUri="voice_service_state";
    /**
    ...
    private ContentObserver mVoiceServiceObserver = new ContentObserver(this){
                 @Override
         public void onChange(boolean selfChange) {
           int voiceState =Settings.Global.getInt(mPhone.getContext().getContentResolver(), mVoiceStateUri,1);
           log("mVoiceServiceObserver voiceState  : "+voiceState);
          // mPhone.getOutgoingCallerIdDisplay(obtainMessage(EVENT_GET_CLIR,EVENT_GET_CLIR,EVENT_GET_CLIR));
           mPhone.setOutgoingCallerIdDisplay(voiceState,obtainMessage(EVENT_SET_CLIR));
         }
     };
    ...
    public ServiceStateTracker(GsmCdmaPhone phone, CommandsInterface ci) {
        ...
        mPhone.getContext().getContentResolver().registerContentObserver(Settings.Global.getUriFor(mVoiceStateUri),
            true, mVoiceServiceObserver);
        ...
        switch (msg.what) {
            ...
            case EVENT_SET_CLIR:
                ...
                if (ar.exception != null) {
                    log( "handleMessage:EVENT_GET_CLIR ar.exception="+ar.exception);
                }else{
                log("handleMessage: EVENT_SET_CLIR Success");
                }
                break;
            ...
            case EVENT_SET_NET_MODE_GLOBAL:
              ...
              case EVENT_SET_ALERT_NET_GLOBAL:
                if(SystemProperties.get("gsm.version.baseband", "null").contains("SC20A") &&
                     !SystemProperties.get("gsm.version.baseband", "null").contains("SC20AU") &&
    ...
    protected void handlePollStateResultMessage(int what, AsyncResult ar) {
                ...
                if (mPhone.isPhoneTypeGsm()) {
                    ...
                    if (states.length > 0) {
                        try {
                            ...
                            if (states.length >= 13 && (states[13] != null)) {
                                reasonRegStateDenied = Integer.parseInt(states[13]);
                                log("SIM or Phone Error : reasonRegStateDenied= "+reasonRegStateDenied);
                                String message=null;
                                switch(reasonRegStateDenied){
                                    case 2:
                                    ...
                                    message="SIM not allowed MM#3";
                                        break;
                                    case 6:
                                    message="Phone not alloweds MM#6";
                                        break;
                                }
                                if(mRadioCertificationMode ==1){
                                log("show alert dialog for radio state error");
                                    alertError(message);
                                }
                            }
                            if (states.length > 14) {
       ...
       private void alertError(String message) {
            ...
            AlertDialog.Builder builder = new AlertDialog
                    .Builder(mPhone.getContext()).setTitle("Reject the registration attept")
                    .setMessage(message);
   ...
   public boolean isSimCardyReady() {
        int state = ((TelephonyManager) mPhone.getContext().
                getSystemService(Context.TELEPHONY_SERVICE)).
                getSimState();
   ...
   private  void alertChangeNetMode(){
       ...
       if(netMode==Phone.NT_MODE_LTE_CDMA_EVDO_GSM_WCDMA){
            ...
            return;
             ...
             AlertDialog.Builder builder = new AlertDialog
            .Builder(mPhone.getContext()).setTitle(Resources.getSystem().
             getText(com.android.internal.R.string.network_not_available_title).toString())
            .setMessage(Resources.getSystem().
             getText(com.android.internal.R.string.network_not_available_message).toString())
