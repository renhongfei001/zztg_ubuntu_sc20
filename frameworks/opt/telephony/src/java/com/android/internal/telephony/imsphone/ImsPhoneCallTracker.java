...
public class ImsPhoneCallTracker extends CallTracker implements ImsPullCall {
    ...
    //***** cody add for control emergency dial
    private EmergencyDialObserver mEmergencyDialObserver;
    private int mEmergencyMode = 1;
    private final String mEmergencyUri="TOGGLE_EMERGENCY_DIAL";
    //***** Events
    ...
    public ImsPhoneCallTracker(ImsPhone phone) {
        ...
        mEmergencyMode=Settings.Global.getInt(mPhone.getContext().getContentResolver(), mEmergencyUri,1);
        mEmergencyDialObserver = new EmergencyDialObserver(new Handler());
        mPhone.getContext().getContentResolver().registerContentObserver(Settings.Global.getUriFor(mEmergencyUri),true, mEmergencyDialObserver);
    }

	private class EmergencyDialObserver extends ContentObserver {
		 /**
		  * Creates a content observer.
		  *
		  * @param handler The handler to run {@link #onChange} on, or null if none.
		  */
		 public EmergencyDialObserver(Handler handler) {
			 super(handler);
		 }
        ...
        public void onChange(boolean selfChange, Uri uri) {
         mEmergencyMode=Settings.Global.getInt(mPhone.getContext().getContentResolver(), mEmergencyUri,1);
         if (DBG) log("Hang Up dial before emergency is : "+(mEmergencyMode==1?"open":"close"));
          }
    ...
    public void dispose() {
        ...
        mPhone.getContext().getContentResolver().unregisterContentObserver(mEmergencyDialObserver);
    ...
    dial(String dialString, int clirMode, int videoState, Bundle intentExtras)
        ...
        if (mForegroundCall.getState() == ImsPhoneCall.State.ACTIVE) {
            ...
            if(isEmergencyCall &&(mEmergencyMode==1)){
               mForegroundCall.hangup();
               Rlog.d(LOG_TAG, "hangup before Emergency Call");
            }else{
